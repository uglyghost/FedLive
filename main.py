...
from utils.utils import display_frames_as_gif
...
if __name__ == '__main__':
    ...
    totalUser = args.saliTestNum + args.saliTrainNum
    ...
    # 训练集
    trainSolvers = solvers[0:args.saliTrainNum]
    train_sali_u = serverSali(trainSolvers)
    # 测试集
    # testSolvers = solvers[args.saliTrainNum:args.saliTrainNum + args.saliTestNum]
    testSolvers = solvers[args.saliTrainNum:args.saliTrainNum + args.saliTestNum]
    ...
    frames = []
    solvers[args.visId - 1].countMain = 0
    for i in range(int(solvers[-1].totalFrames/8)):
        view_point = env.getNextSate(solvers[-1])
        view_point_fix = []
        for index, value in enumerate(view_point):
            view_point_fix.append([value[0] / solvers[-1].W_Frame, value[1] / solvers[-1].H_Frame])

        next_vec = stateToVec(solvers[-1], view_point)
        if sum(next_vec) == 0:
            continue
    ...
    # 保存到gif
    display_frames_as_gif('Real', frames)
    ...
    for i, saliency in enumerate(resultSali[1:-1]):
        ...
        for j, viewer in enumerate(testSolvers):
            ...
            if j == 0:
                ...
                agents[0].save_checkpoint()
            else:
                similarity = np.zeros(args.saliTestNum)
                for index, value in enumerate(curr_state_c):
                    for i, x in enumerate(value):
                        if x == curr_state[i]:
                            similarity[index] += 1

                most_match_index = np.argmax(similarity)
                if similarity[most_match_index] > args.threshold:
                    agents[j].load_checkpoint(policy_model=agents[most_match_index].policy_model)
                prediction_vec, rewardTmp = agents[j].train_one_epoch(curr_state, next_state)
                agents[j].save_checkpoint()

            curr_state_c.append(curr_state)
            ...
            if args.visId == viewer.userId:
                ...
                frames.append(env[j].render(mode='rgb_array'))
                # env[j].render()
