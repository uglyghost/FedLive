...
class ReplayBuffer(object):
    def __init__(self, max_size, state_dim, action_dim):
        ...
        self.action_memory = np.zeros((self.mem_size, *[action_dim]), dtype=np.int64)
...
class DDQN(object):
    def __init__(self,
        ...
        self.memory = ReplayBuffer(mem_size, self.state_dim, action_dim)

        self.q_eval = DuelingLinearDeepQNetwork(alpha, action_dim, state_dim=self.state_dim,
                                                name='q_eval', chkpt_dir=chkpt_dir)
        self.q_next = DuelingLinearDeepQNetwork(alpha, action_dim, state_dim=self.state_dim,
                                                name='q_next', chkpt_dir=chkpt_dir)

    def store_transition(self, curr_state, action, next_state, reward, done):
        self.memory.store_transition(curr_state, action, reward, next_state, done)
    ...
    def select_action(self, observation):
        if np.random.random() > self.epsilon:
            ...
            action = advantage.cpu().detach().numpy().reshape([200])
        else:
